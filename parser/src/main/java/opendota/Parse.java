...
public class Parse {
    ...
    boolean startTimeEmitted = false;
        ...
        try {
            ...
            if (combatLogEntry.type.equals("DOTA_COMBATLOG_GAME_STATE") && combatLogEntry.value == 5) {
                ...
                if (gameStartTime == 0) {
                    ...
                    flushLogBuffer();
                     Entry dateEntry = new Entry(gameStartTime); // 👈 додай
        dateEntry.type = "game_start_time";
        dateEntry.game_start_time = (long) gameStartTime;
        output(dateEntry);
                }
            }
    ...
    public void onTickStart(Context ctx, boolean synthetic) {
        ...
        if (grp != null) {
                    int startTime = Math.round((float) grp.getProperty("m_pGameRules.m_flGameStartTime")); 
     if (startTime != 0 && !startTimeEmitted) {
        Entry dateEntry = new Entry(time);
        dateEntry.type = "game_start_time";
        dateEntry.game_start_time = (long) startTime;
        output(dateEntry);
        startTimeEmitted = true;
    }
    ...
    Entry dateEntry = new Entry(time);
    dateEntry.type = "game_start_time";
    dateEntry.game_start_time = (long) gameStartTime; // 👈 змінив це
    output(dateEntry);
}
            if (draftStage == 2) {
                ...
                while (added < numPlayers && i < 30) {
                    try {
                        ...
                        if (playerTeam == 2 || playerTeam == 3) {
                            ...
                            entry.accountid = steamid - 76561197960265728L;
